...
var mutateType = graphql.NewObject(graphql.ObjectConfig{
	...
	Fields: graphql.Fields{
		...
		"deleteUserInGroup": &graphql.Field{
			Type: graphql.Boolean,
			Args: graphql.FieldConfigArgument{
				"emailUser": &graphql.ArgumentConfig{
					Type: graphql.String,
				},
		...
		"createReport": &graphql.Field{
			...
			Resolve: func(p graphql.ResolveParams) (interface{}, error) {
				...
				emailUser := p.Args["emailUser"].(string)
				nameGroup := p.Args["nameGroup"].(string)
				summerization := p.Args["summerization"].(string)


				return CreateReport(contentTodoes, states, summerization, emailUser, nameGroup), nil
			},
		},
